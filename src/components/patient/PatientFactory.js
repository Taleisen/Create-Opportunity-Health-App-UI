...
import styles from './PatientFactory.module.css';
import {
  isValidName,
  isValidEmail,
  isValidSsn,
...
/**
 * generates new patient to save in database
 * @returns new patient saved to database
 */
function PatientFactory() {
  ...
  const validateValues = () => {
    ...
    if (!isValidStateFormat(stateInput.value) || !isValidState(stateInput.value)) {
      setStateInput({
        ...stateInput,
        error: true,
      });
  ...
  /**
   * handles submission of data to database after verification
  ...
  return (
    <div>
      ...
      <Link to="/patients"><button type="button" className={styles.button}>Return to Patients List</button></Link>
    </div>
  );
}
