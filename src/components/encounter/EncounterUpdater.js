...
import {
  isValidVisit, isValidBilling, isValidIcd, isValidNumber, isValidDate,
} from '../../validation';

/**
 * updates an encounter and saves updated encounter to database
 * @returns Encounter updated in database
 */
function EncounterUpdater() {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateValues()) {
      ...
      axios({
        method: 'put',
        url: `http://localhost:8080/patients/${current.patientId}/encounters/${params.id}`,
        data: body,
      }).then(navigate(`/patients/${current.patientId}/encounters/${params.id}`));
    }
  };
  ...
  function mapEncounter() {
    axios.get(`http://localhost:8080/patients/:id/encounters/${params.id}`)
      .then(
        (response) => {
          setNotesInput({ ...notesInput, value: response.data.notes });
          setVisitCodeInput({ ...visitCodeInput, value: response.data.visitCode });
          setProviderInput({ ...providerInput, value: response.data.provider });
          setBillingCodeInput({ ...billingCodeInput, value: response.data.billingCode });
          seticd10Input({ ...icd10Input, value: response.data.icd10 });
          setTotalCostInput({ ...totalCostInput, value: response.data.totalCost });
          setCopayInput({ ...copayInput, value: response.data.copay });
          setChiefComplaintInput({ ...chiefComplaintInput, value: response.data.chiefComplaint });
          setPulseInput({ ...pulseInput, value: response.data.pulse });
          setSysInput({ ...sysInput, value: response.data.systolic });
          setDiaInput({ ...diaInput, value: response.data.diastolic });
          setDateInput({ ...dateInput, value: response.data.date });
          setCurrent(response.data);
        },
      );
  }
  useEffect(() => { mapEncounter(); }, []);
  ...
  return (
    <div>
      ...
      <Link to={`/patients/${current.patientId}/encounters/${current.id}`}><button type="button" className={styles.button}>Return to Encounter Details</button></Link>
    </div>
  );
}

export default EncounterUpdater;
